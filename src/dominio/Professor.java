...
//        Collections.sort(this.grade, new AulaComparator());
    ...
    private void avaliar(Aula aulaPedida, Interval primeiroDia, String motivo, List<Ausencia> ausencias) {
                ...
                && aulaPedida.bateHorarioCom(primeiroDia)) 
        {
            ...
            int inícioHoraAula = aulaPedida.getPeriodo().getStart()
                    .getHourOfDay();
            int inícioMinutoAula = aulaPedida.getPeriodo().getStart()
                    .getMinuteOfHour();
            int finalHoraAula = aulaPedida.getPeriodo().getEnd()
                    .getHourOfDay();
            int finalMinutoAula = aulaPedida.getPeriodo().getEnd()
                    .getMinuteOfHour();

            DateTime inícioAusênciaAula = new DateTime(anoAusência,mêsAusência
                    ,diaAusência,inícioHoraAula,inícioMinutoAula);
            DateTime finalAusênciaAula = new DateTime(anoAusência,mêsAusência
                    ,diaAusência,finalHoraAula,finalMinutoAula);
            Interval períodoAusência = new Interval(inícioAusênciaAula
                    ,finalAusênciaAula);
